...
public class GameSaveService {
    ...
    public List<GameSaveResponse> getAll(String login) {
        ...
        return saveRepository
                .findByUserIdOrderBySlotIndexAsc(user.getId())
                .stream()
                .map(this::toResponse)
                .toList();
    ...
    public GameSaveResponse upsert(
            String login,
            GameSaveUpsertRequest request) {
        ...
        save.setSlotName(
                normalizeOrDefault(
                        request.slotName(),
                        slotLabel(request.slotIndex())));

        save.setSelectedCharacter(
                normalizeOrDefault(
                        request.selectedCharacter(),
                        "Warrior"));

        save.setPlayTutorial(
                request.playTutorial()
                        && request.playTutorial());
        ...
        save.setCheckpointId(
                normalizeOrDefault(
                        request.checkpointId(),
                        ""));

        save.setCheckpoint(
                valueInt(request.checkpoint()));

       

        save.setCollectedIdsJson(
                toJsonArray(
                        request.collectedIds()));

        save.setDeadEnemyIdsJson(
                toJsonArray(
                        request.deadEnemyIds()));

        save.setCompletionPercent(
                valueFloat(
                        request.completionPercent()));

        save.setQttAppleCollected(
                valueInt(
                        request.qttAppleCollected()));

        save.setQttGlassCollected(
                valueInt(
                        request.qttGlassCollected()));

        save.setQttPlasticCollected(
                valueInt(
                        request.qttPlasticCollected()));

        save.setQttEletronicsCollected(
                valueInt(
                        request.qttEletronicsCollected()));

        save.setQttPaperCollected(
                valueInt(
                        request.qttPaperCollected()));

        save.setQttMetalCollected(
                valueInt(
                        request.qttMetalCollected()));
    ...
    private GameSaveResponse toResponse(GameSave save) {
        ...
        return new GameSaveResponse(
                ...
                save.getQttEletronicsCollected(),
